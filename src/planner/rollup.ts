...
export function rollupWeek(g: Graph, week: string, orders: Order[], supply: Supply[]): WeekPlan {
  ...
  const byproducts: ByproductCoverage[] = [...demand].map(([product, demandKg]) => {
    ...
    return { product, demandKg, outputKg, shortfallKg, coveredByExtraFishKg: Math.min(shortfallKg, Math.max(0, outputFor(product) - outputKg)), extraRawKg: bought?.extraRawKg ?? 0, fallsTo: bought?.fallsTo }
  })
